...
def build_ai_config_tab(self):
    ...
    base_url_entry.grid(row=row_start, column=1, columnspan=2, padx=5, pady=5, sticky="nsew")
    ...
    interface_dropdown.grid(row=row_start+1, column=1, columnspan=2, padx=5, pady=5, sticky="nsew")

    create_label_with_help(self, self.ai_config_tab, "模型名称:", "model_name", row_start+2, 0)
    ...
    model_name_entry.grid(row=row_start+2, column=1, columnspan=2, padx=5, pady=5, sticky="nsew")

    create_label_with_help(self, self.ai_config_tab, "Temperature:", "temperature", row_start+3, 0)
    ...
    temp_scale.grid(row=row_start+3, column=1, padx=5, pady=5, sticky="we")
    self.temp_value_label = ctk.CTkLabel(
        self.ai_config_tab,
        text=f"{self.temperature_var.get():.2f}",
        font=("Microsoft YaHei", 12)
    )
    self.temp_value_label.grid(row=row_start+3, column=2, padx=5, pady=5, sticky="w")

    create_label_with_help(self, self.ai_config_tab, "Max Tokens:", "max_tokens", row_start+4, 0)
    ...
    max_tokens_slider.grid(row=row_start+4, column=1, padx=5, pady=5, sticky="we")
    self.max_tokens_value_label = ctk.CTkLabel(
        self.ai_config_tab,
        text=str(self.max_tokens_var.get()),
        font=("Microsoft YaHei", 12)
    )
    self.max_tokens_value_label.grid(row=row_start+4, column=2, padx=5, pady=5, sticky="w")

    create_label_with_help(self, self.ai_config_tab, "Timeout (sec):", "timeout", row_start+5, 0)
    ...
    timeout_slider.grid(row=row_start+5, column=1, padx=5, pady=5, sticky="we")
    self.timeout_value_label = ctk.CTkLabel(
        self.ai_config_tab,
        text=str(self.timeout_var.get()),
        font=("Microsoft YaHei", 12)
    )
    self.timeout_value_label.grid(row=row_start+5, column=2, padx=5, pady=5, sticky="w")
    ...
    test_btn.grid(row=row_start+6, column=0, columnspan=3, padx=5, pady=5, sticky="ew")
...
def build_embeddings_config_tab(self):
    def on_embedding_interface_changed(new_value):
        ...
        if self.loaded_config and "embedding_configs" in self.loaded_config and new_value in self.loaded_config["embedding_configs"]:
            ...
            if new_value == "阿里云百炼":
                self.embedding_url_var.set("https://dashscope.aliyuncs.com/compatible-mode/v1")
                self.embedding_model_name_var.set("text-embedding-v2")
...
def build_config_choose_tab(self):
    ...
    def save_config_choose():
        ...
        if not config_data_full:
            config_data_full = {}
        if "choose_configs" not in config_data_full:
            config_data_full["choose_configs"] = {}
        config_data_full["choose_configs"]["architecture_llm"] = self.architecture_llm_var.get()
...
def load_config_btn(self):
    ...
    if cfg:
        ...
        if llm_configs:
            ...
            self.temp_value_label.configure(text=f"{float(llm_conf.get('temperature', 0.7)):.2f}")
            self.max_tokens_value_label.configure(text=str(int(llm_conf.get('max_tokens', 8192))))
            self.timeout_value_label.configure(text=str(int(llm_conf.get('timeout', 600))))

        last_embedding = cfg.get("last_embedding_interface_format", "阿里云百炼")
